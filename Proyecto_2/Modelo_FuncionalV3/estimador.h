      ...
      while (true) { 
        ...
        if(address == I_scale_factor_Addr){
          //Pasar de puntero a Dato, pasarlo a flotante
		  	...
		  	else if(address == V_scale_factor_Addr){
		  		V_scale_factor = data;
		  		cout << "I scale" << I_scale_factor << endl;
		  	}
		  	else if(address == V_scale_factor_Addr){
		  	...
		  	else if(address == Ig_value_Addr){
		  		Ig = data;
		  		cout << "Ig" << Ig << endl;
		  	}
		  	else if(address == Gamma11_Addr){
		  		GAMMA11 = data;
		  		cout << "GAMMA11" << GAMMA11 << endl;
		  	}
		  	else if(address == Gamma12_Addr){
		  		GAMMA12 = data;
		  		cout << "GAMMA12" << GAMMA12 << endl;
		  	}
		  	else if(address == Gamma21_Addr){
		  		GAMMA21 = data;
		  		cout << "GAMMA21" << GAMMA21 << endl;
		  	}
		  	else if(address == Gamma22_Addr){
		  		GAMMA22 = data;
		  		cout << "GAMMA22" << GAMMA22 << endl;
		  	}
		  	else if(address == Init_alpha_Addr){
		  		INIT_ALPHA = data;
		  		cout << "INIT_ALPHA" << INIT_ALPHA << endl;
		  	}
		  	else if(address == Init_beta_Addr){
		  		INIT_BETA = data;
...
*/
  ...
  sc_event_queue do_t;
  sc_event  done_t, auxC;
  int data;  
  long int addrs;
  bool comando;  

  //Variables de puerto Target
  sc_event  target_t, initiator_done_t;
